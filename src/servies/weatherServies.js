...
const formatToLocalTime = (secs, offest, format = "cccc, dd LLL yyyy' | Local time: 'hh:mm a") => DateTime.fromSeconds(secs + offest, { zone: "utc" }).toFormat(format);
...
const formatCurrent = (data) => {
    ...
    return {
        lat, lon, temp, feels_like, temp_min, temp_max, humidity, name, country, sunrise: formatToLocalTime(sunrise, timezone, "hh:mm a"),
...
const formattForecastWeather = (secs, offest, data) => {
    const hourly = data.filter((f)=>f.dt>secs).map((d) => (
        {
            temp: d.main.temp,
    ...
    const daily = data.filter((f) => f.dt_txt.slice(-8) === "00:00:00").map((d) => ({
        temp: d.main.temp,
...
const getFormatedWeatherData = async (searchParams) => {
    const formatedCurrentWeather = await getWeatherData('weather', searchParams).then(data => formatCurrent(data));

    const { dt, lat, lon, timezone } = formatedCurrentWeather;

    const formattedForecastWeather = await getWeatherData("forecast", { lat, lon, units: searchParams.units }).then((data) => 
     
        formattForecastWeather(dt, timezone, data.list));
